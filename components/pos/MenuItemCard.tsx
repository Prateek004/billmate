...
export default function MenuItemCard({ item, onConfigPress }: Props) {
  ...
  // Fast-add: items with no sizes, portions, or add-ons, OR explicitly fastAdd
  const hasOptions =
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 mb-2">
        ...
        {!item.isAvailable && (
          <span className="text-[9px] text-gray-400 font-bold uppercase tracking-wide">Unavailable</span>
        )}
      </div>
      ...
      {/* Configurable hint */}
      {hasOptions && !item.fastAdd && (
        <p className="text-[10px] text-primary-500 font-semibold mt-1">
          {item.sizes ? "Sizes •" : ""}{item.portions ? " Portions •" : ""}{item.addOns?.length ? " Add-ons" : ""}
        </p>
      )}
    </button>
  );
